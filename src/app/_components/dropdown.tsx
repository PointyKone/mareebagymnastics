...
export function Dropdown({ name, items }: { name: string, items: { name: string, href: string }[] }) {
    ...
    return (
        <div className="my-auto relative">
            <button onMouseLeave={() => setOpen(false)} onMouseOver={() => setOpen(true)} className="pl-8 font-extralight flex" key={name}>{name}<span><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 pl-2 h-full my-auto">
            ...
            <div onMouseLeave={() => setOpen(false)} onMouseOver={() => setOpen(true)} className={`${open ? "flex opacity-100 z-10" : "invisible opacity-0"} absolute flex-col flex justify-end text-center bg-white p-8 shadow-md rounded-md left-[-16px] top-12 transition-all ease-in-out duration-150 `}>
                {items.map(item => {
                    ...
                    return (
                        <a className="py-2 font-extralight" href={item.href}>{item.name}</a>
                    )
                })}
            </div>
        </div>
